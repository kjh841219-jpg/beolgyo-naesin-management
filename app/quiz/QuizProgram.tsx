...
export default function QuizProgram() {
  ...
  useEffect(()=>{if(!student?.id||!progressReady)return;const timer=window.setTimeout(()=>void fetch("/api/student/quiz-progress",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({area:"passage",progress:{type,passage,index,answer,built,checked,score,solved,publisher:currentPublisher,grade:currentGrade,lesson:currentLesson,title:current.title}})}),700);return()=>window.clearTimeout(timer)},[student?.id,progressReady,type,passage,index,answer,built,checked,score,solved,currentPublisher,currentGrade,currentLesson,current.title]);
