...
export default function FullBlankQuiz() {
  ...
  useEffect(()=>{if(!student?.id||!progressReady)return;const timer=window.setTimeout(()=>void fetch("/api/student/quiz-progress",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({area:"blank",progress:{passage,round,filled:Object.values(answers).filter(Boolean).length,checked,score,solved,publisher,grade,lesson,title:current.title}})}),700);return()=>window.clearTimeout(timer)},[student?.id,progressReady,passage,round,answers,checked,score,solved,publisher,grade,lesson,current.title]);
