...
export default function WordQuizProgram() {
  ...
  const printCurrentWord = () => {
    ...
    popup.document.write(`<!doctype html><html lang="ko"><head><meta charset="utf-8"><title>${label} 문제</title><style>@page{size:A4;margin:20mm}body{font-family:'Malgun Gothic',sans-serif;color:#152b42}h1{font-size:22px}.meta{color:#66758a;margin-bottom:35px}.card{padding:30px;border:1px solid #bfcbd6;border-radius:12px}.card b{display:block;color:#147a63;margin-bottom:18px}.card p{font-size:24px}.line{height:42px;border-bottom:1px solid #666}</style></head><body><h1>벌교미래엔영어 단어퀴즈 · ${label}</h1><div class="meta">${current.publisher} · ${current.grade} · ${current.lesson}<br>이름: ____________ 날짜: ____________</div><section class="card"><b>${position + 1}번 문제</b><p>${prompt}</p><div class="line"></div><div class="line"></div></section></body></html>`);
